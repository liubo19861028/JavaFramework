...
/**
 * 
 * 接口或类的说明: 图片处理工具类 功能：缩放图像、切割图像、图像类型转换、彩色转黑白、文字水印、图片水印等 <br>
 * ========================== <br>
 * 公司：南京壹号家信息科技有限公司 <br>
...
public class ImageUtil {
	...
	public final static void scale2(String srcImageFile, String result, int height, int width, boolean bb) {
		try {
			...
			if (1 == 1) {
				// 为等比缩放计算输出的图片宽度及高度
				double rate1 = ((double) itemp.getWidth(null)) / (double) width + 0.1;
				double rate2 = ((double) itemp.getHeight(null)) / (double) height + 0.1;
				double rate = rate1 > rate2 ? rate1 : rate2;
				new_w = (int) (((double) itemp.getWidth(null)) / rate);
				new_h = (int) (((double) itemp.getHeight(null)) / rate);
			}
	 ...
	 * @throws Exception
	 */
	public static String scale3(String srcImageFile, String result, int w, int h, boolean percentage) throws Exception {
		if (srcImageFile != null && w > 0 && h > 0) {
